    ...
    for line in cards_list:
        try:
            if len(line) > 2:
                ...
                count += num_copies
                if parse_str:
                    cleaned_cards_str += f"{num_copies} {line.title()}\n"
        except ValueError:
            pass
    if parse_str:
...
"""
    ...
    html_deck = f"""<table class="deck" style="width: 70%;  background-color: #ecf3f7;" cellspacing="7">
        ...
        <tr>
            <td valign="top">
            """
    for index, line in enumerate(cards_splitted_by_type.replace("[/mazzo][/td][td][mazzo]", "</td><td valign='top'>").splitlines()):
        if line[-2:] == "):":
            if index != 0:
                html_deck += "</div>"
            html_deck += f"<div>{line}<br>"
        elif len(line) < 2:
            continue
        elif line == "</td><td valign='top'>":
            html_deck += line
        else:
